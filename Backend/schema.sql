-- SafeWay London database setup using PostgreSQL and PostGIS.
-- careful: running this again drops and recreates all tables.
...
-- 1. crimes  (no foreign key, matched to the legs by location)
...
-- index to make the distance queries faster
...
-- unique on the lowercase email, so Ana@x.com and ana@x.com
...
-- 8. tfl_api_cache  (no foreign key, just a cache)
