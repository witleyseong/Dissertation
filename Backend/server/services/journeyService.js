...
// short cache (2 min) because TfL times are live,
// it's just so the same search twice in a row doesn't call TfL again
const TFL_CACHE_TTL_MS = 2 * 60 * 1000;

// error types so the controller can send the right status code
class TflConfigError extends Error { }   // bad setup = 500
class TflTimeoutError extends Error { }  // TfL too slow = 503
class TflUpstreamError extends Error {  // TfL returned an error = 502
    constructor(message, status) {
        super(message);
        this.upstreamStatus = status;
    }
}
// thrown when TfL finds more than one place for the text the user typed
class TflDisambiguationError extends Error {
    constructor(disambiguation) {
        super("origin and/or destination text matched multiple TfL locations");
        this.disambiguation = disambiguation; // { origin: Candidate[]|null, destination: Candidate[]|null }
    }
}
...
// risk level (the iOS app still uses this one)
// it is based on past recorded crimes, not the chance something happens to you
...
// score from 0 to 100, more crimes per km = lower score
...
// TfL gives [lat, lng] but PostGIS wants [lng, lat], so swap them
...
function buildTflUrl(from, to) {
    ...
    // the key is optional, TfL still answers without it (just a lower rate limit)
...
// turns TfL's list of matching places into name + coordinates
function extractCandidates(disambiguation) {
    const options = disambiguation?.disambiguationOptions || [];
    return options
        .map((opt) => ({
            name: opt.place?.commonName || opt.place?.name || null,
            lat: opt.place?.lat,
            lon: opt.place?.lon,
        }))
...
// hash of the search, used as the key in tfl_api_cache
function tflCacheKey(from, to) {
    const normalized = `${from.trim().toLowerCase()}|${to.trim().toLowerCase()}|${TFL_MODES}`;
    return crypto.createHash("sha256").update(normalized).digest("hex");
}

async function getCachedTflResponse(requestHash) {
    const { rows } = await pool.query(
        `SELECT raw_response FROM tfl_api_cache WHERE request_hash = $1 AND expires_at > now()`,
        [requestHash]
    );
    return rows[0]?.raw_response ?? null;
}

// only saves successful responses, and hides the api key in the saved url
async function storeCachedTflResponse(requestHash, requestUrl, rawResponse) {
    const expiresAt = new Date(Date.now() + TFL_CACHE_TTL_MS);
    await pool.query(
        `INSERT INTO tfl_api_cache (request_hash, request_url, raw_response, expires_at)
         VALUES ($1, $2, $3, $4)
        ...
        [requestHash, redactApiKey(requestUrl), JSON.stringify(rawResponse), expiresAt]
    );
}
...
async function fetchTflJourney(from, to) {
    const requestHash = tflCacheKey(from, to);
    try {
        const cached = await getCachedTflResponse(requestHash);
        if (cached) return cached;
    } catch (err) {
        // if the cache fails just call TfL normally
        console.error("tfl_api_cache read failed, continuing without cache:", err.message);
    }

    const requestUrl = buildTflUrl(from, to);
    ...
    if (response.status === 300) {
        ...
        const fromCandidates = extractCandidates(body.fromLocationDisambiguation);
        const toCandidates = extractCandidates(body.toLocationDisambiguation);
        const originAmbiguous = fromCandidates.length > 1;
        const destinationAmbiguous = toCandidates.length > 1;

        // more than one match: ask the user instead of guessing
        if (originAmbiguous || destinationAmbiguous) {
            throw new TflDisambiguationError({
                origin: originAmbiguous ? fromCandidates : null,
                destination: destinationAmbiguous ? toCandidates : null,
            });
        }

        // 0 or 1 match means there is nothing to choose, so use it and try again
        const resolvedFrom = fromCandidates[0] ? `${fromCandidates[0].lat},${fromCandidates[0].lon}` : from;
        const resolvedTo = toCandidates[0] ? `${toCandidates[0].lat},${toCandidates[0].lon}` : to;
        response = await fetchWithTimeout(buildTflUrl(resolvedFrom, resolvedTo));
    }

    // 404 just means no route was found, not a real error
    if (response.status === 404) {
        const empty = { journeys: [] };
        try {
            await storeCachedTflResponse(requestHash, requestUrl, empty);
        } catch (err) {
            console.error("tfl_api_cache write failed, continuing without cache:", err.message);
        }
        return empty;
    }

    if (!response.ok) {
        // log TfL's error text but don't send it to the client
    ...
    try {
        await storeCachedTflResponse(requestHash, requestUrl, json);
    } catch (err) {
        console.error("tfl_api_cache write failed, continuing without cache:", err.message);
    }
...
async function buildLeg(rawLeg) {
    ...
    // only walking legs get a crime count, the rest are null
    ...
    return {
        ...
        // only used inside buildJourney, removed before sending
        _matches: matches,
    };
}
...
async function buildJourney(rawJourney, index) {
    ...
    // the buffers of two legs can overlap, so count each crime once
    ...
    // no walking = no score (better than showing a fake one)
    ...
    return {
        ...
        // same as exposureScore, kept for the iOS app
...
async function planJourneys(from, to) {
    ...
    // mark the fastest and the lowest exposure journeys (ties are all marked)
    ...
    // compare crimes per km, not the total, because a longer walk has more crimes
    // just from being longer
    const measured = journeys.filter((j) => j.totalWalkingKm > 0 && j.exposurePerKm !== null);
    const minExposurePerKm = measured.length > 0 ? Math.min(...measured.map((j) => j.exposurePerKm)) : null;

    for (const journey of journeys) {
        journey.isFastest = minDuration !== null && journey.duration === minDuration;
        journey.isLowestExposure = minExposurePerKm !== null && journey.exposurePerKm === minExposurePerKm;
    }

    return journeys;
}

// saves the journeys in route_options, route_legs and route_scores
// in one transaction so nothing is half saved
...
// saves the request first, then calls TfL and saves the routes for the history
// if TfL fails the request stays with status 'error'
async function planJourneysForUser(userId, from, to) {
    ...
    try {
        ...
        // ambiguous is not a failure
        const status = err instanceof TflDisambiguationError ? "ambiguous" : "error";
        await pool.query(`UPDATE journey_requests SET status = $2 WHERE id = $1`, [journeyRequestId, status]);
        throw err;
    }
...
// last journeys of the user with their routes
