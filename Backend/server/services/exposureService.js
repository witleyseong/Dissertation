...
// finds the crimes near a walking leg and the length of the leg
// journeyService uses this too, to avoid counting the same crime twice
async function getLegCrimeMatches(legCoordinates, bufferMeters){
  ...
  // the join can return the same crime twice, so use a Set
  const seen = new Set();
  const matches = [];
...
// crimes per km (with a minimum distance so short legs don't blow up)
